...
export default function MissionDetail() {
  ...
  const refresh = async () => {
    const d = (await api.get(`/campaigns/${id}`)).data;
    setM(d);
  ...
  const override = async (action) => {
    if (!m) return;
    if (action === "resume") {
      await api.post(`/campaigns/${id}/state`, { state: "resume" });
    } else if (action === "pause") {
      await api.post(`/campaigns/${id}/state`, { state: "paused" });
    } else if (action === "abort") {
      await api.post(`/campaigns/${id}/state`, { state: "abort" });
    }
    await refresh();
  };
  ...
  const addInsight = async () => {
    ...
    await api.patch(`/campaigns/${id}`, { insights_rich: next });
    setNewInsight("");
    await refresh();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => navigate("/campaigns")} className="px-3 py-1 bg-neutral-200 rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-500">Back to Missions</button>
      </div>
