...
export default function HotLeads() {
  ...
  const refresh = async () => {
    ...
    setItems(list);
    setPending(list.filter((h) => h.status === "pending_approval"));
  };
  useEffect(() => { refresh(); }, []);
        ...
        <div className="grid gap-2">
          {pending.map((h) => (
            <div key={h.id} className="border rounded p-2">
              ...
              <div className="text-xs text-neutral-500">{phoenixTime(h.created_at)}</div>
          ...
          <tbody>
            {items.map((h) => (
              <tr key={h.id} className="border-t">
                ...
                <td className="p-2">{h.prospect_id.slice(0,8)}</td>
