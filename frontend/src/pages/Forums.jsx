  ...
  return (
    <div>
      ...
      <div className="bg-white rounded shadow p-3 mb-4 grid grid-cols-1 md:grid-cols-5 gap-2">
        <input className="border rounded px-2 py-1" placeholder="Platform" value={form.platform} onChange={(e) => setForm({ ...form, platform: e.target.value })} />
        ...
        <select className="border rounded px-2 py-1" value={form.rule_profile} onChange={(e) => setForm({ ...form, rule_profile: e.target.value })}>
          <option value="strict_help_only">strict_help_only</option>
          <option value="open_soft_marketing">open_soft_marketing</option>
        </select>
        <div className="flex gap-1">
          <input className="border rounded px-2 py-1 flex-1" placeholder="topic tag" value={tagInput} onChange={(e) => setTagInput(e.target.value)} />
          <button className="px-2 py-1 bg-neutral-800 text-white rounded" onClick={addTag}>Add</button>
        </div>
          ...
          <tbody>
            {items.map((f) => (
              <tr key={f.id} className="border-t">
                ...
                <td className="p-2"><a href={f.url} target="_blank" rel="noreferrer" className="text-blue-600 underline">{f.name}</a></td>
                <td className="p-2">{f.rule_profile}</td>
                <td className="p-2">{(f.topic_tags||[]).join(", ")}</td>
                <td className="p-2">{phoenixTime(f.last_seen_at)}</td>
